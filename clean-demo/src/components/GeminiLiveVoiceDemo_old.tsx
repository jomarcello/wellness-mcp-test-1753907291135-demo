...
export default function GeminiLiveVoiceDemo() {
  ...
  // Simple audio playback - skip chunks if already playing
  const playAudioResponse = useCallback(async (base64Audio: string) => {
    try {
      // Skip if already speaking to prevent overlaps
      if (isSpeaking) {
        return;
      }
      ...
      // Create WAV header for 24kHz PCM
      ...
      const wavFile = new Uint8Array(header.byteLength + audioBuffer.length);
      wavFile.set(new Uint8Array(header), 0);
      wavFile.set(audioBuffer, header.byteLength);
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 rounded-lg border border-purple-200">
        ...
        <ul className="space-y-2 text-sm text-purple-700">
          <li>1. เปิด Python server ก่อน: <code className="bg-purple-100 px-2 py-1 rounded">python python-backend/official_gemini_server.py</code></li>
        ...
        <div className="mt-4 p-3 bg-purple-100 rounded-lg">
          <p className="text-xs text-purple-600">
            <strong>หมายเหตุ:</strong> ต้องติดตั้ง Python dependencies ก่อน: <code>pip install google-genai websockets</code>
          </p>
        </div>
      </div>
    </div>
